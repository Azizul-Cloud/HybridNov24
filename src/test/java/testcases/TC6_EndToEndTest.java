 ...
 * 
 * Steps: 1) Perform Account Registration 2) Logout after successful
 * registration 3) User Login with the registered email 4) Search product and
 * add to cart 5) Verify Shopping Cart contents 6) Perform Checkout process //
 * feature not available since it is demo site
 */
...
public class TC6_EndToEndTest extends BaseClass {
	...
	public void executeEndToEndTest() throws InterruptedException {
		...
		// performCheckout(); //feature not available since it is demo site. so
		// commented
	...
	private void performLogout() throws InterruptedException {
		...
		logger.info(isHomePageVisible ? "Logout successful, navigated to Home Page." : "Logout verification failed.");
	}
	...
	private void performCheckout() throws InterruptedException {
		...
		// The next steps for guest checkout filling are commented out due to demo
		// limitations
	...
	/*
	 * private void fillGuestDetails(CheckoutPage checkoutPage) throws
	 * InterruptedException {
	 * checkoutPage.setFirstName(generateString().toUpperCase());
	 * Thread.sleep(1000); checkoutPage.setLastName(generateString().toUpperCase());
	 * Thread.sleep(1000); checkoutPage.setAddress1("address1"); Thread.sleep(1000);
	 * checkoutPage.setAddress2("address2"); Thread.sleep(1000);
	 * checkoutPage.setCity("Delhi"); Thread.sleep(1000);
	 * checkoutPage.setPin("500070"); Thread.sleep(1000);
	 * checkoutPage.setCountry("India"); Thread.sleep(1000);
	 * checkoutPage.setState("Delhi"); Thread.sleep(1000);
	 * checkoutPage.clickOnContinueAfterBillingAddress(); Thread.sleep(1000);
	 * checkoutPage.clickOnContinueAfterDeliveryAddress(); Thread.sleep(1000);
	 * checkoutPage.setDeliveryMethodComment("testing..."); Thread.sleep(1000);
	 * checkoutPage.clickOnContinueAfterDeliveryMethod(); Thread.sleep(1000);
	 * checkoutPage.selectTermsAndConditions(); Thread.sleep(1000);
	 * checkoutPage.clickOnContinueAfterPaymentMethod(); Thread.sleep(2000); }
	 */
}
